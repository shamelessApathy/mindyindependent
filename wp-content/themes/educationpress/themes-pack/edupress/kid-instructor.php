...
//$class_ids = $instructor->get_assigned_classs_ids('publish');
global $edupress_options;
?>
...
<div class="page-spacer clearfix"> 
 <div id="primary">
        <div class="container">
        	<div class="row">
                <main id="main" class="site-main woocommerce col-xs-12<?php echo esc_attr(edupress_page_layout('edupress_classinstructor_layout'));?>" >
             ...
             <?php /* Start the Loop */ 
				...
				'meta_query' => array 
					(
						array(
							'key'     => 'edupress_kid_class_instructors',
							'value'   => '[[:<:]]'.$author_id.'[[:>:]]',
							'compare'   => 'REGEXP',
				...
				$class_query = new WP_Query( $post_args );
			...
			if ( $class_query->have_posts() ) : ?>	
				<div class="col-xs-12 zoom courses grid-group-item">
                    ...
                    $sigle_class_class = edupress_column_class( 'edupress_classinstructor_layout' );

					while ( $class_query->have_posts() ) {
						global $single_product_class; 
						$single_product_class = edupress_column_class('edupress_classinstructor_layout'); 
		...
		<?php else : ?>
			...
			<?php
			esc_html_e( 'No Classes found behalf of this teacher.', 'edupress' );
			?>
