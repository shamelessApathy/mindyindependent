	...
	<div id="primary" class="content-area">
    	<div class="container">
        	<div class="row">
				...
				?>
                    <main id="main" class="site-main col-xs-12<?php echo esc_attr(edupress_page_layout('edupress_blogdetail_layout'));?>" >
                    ...
                    <?php while ( have_posts() ) : the_post(); ?>
                      ...
                      <?php 
					   $orig_post ='';
					  if ( $edupress_options[ 'edupress_related_posts' ] ) {
						  $orig_post = $post;
						...
						if (!empty( $edupress_options[ 'edupress_related_posts_type' ] )&&  $edupress_options[ 'edupress_related_posts_type' ] == 'category' ) {
						...
						}elseif ( !empty( $edupress_options[ 'edupress_related_posts_type' ] )  && $edupress_options[ 'edupress_related_posts_type' ] == 'tag' ) {
                      ...
                      <?php   if ( ! empty( $edupress_options['edupress_related_posts_title'] ) ) {?> 
                      	<h3><?php echo esc_html( $edupress_options['edupress_related_posts_title'] ); ?></h3>
                        <?php }
						while( $related_posts_query->have_posts() ) {
						$related_posts_query->the_post();
						$reltscat=get_the_category($post->ID); ?>
                        <div class="col-xs-12 <?php echo esc_attr(edupress_column_class('edupress_blogdetail_layout'));?>">
                        	<article>
                               <a href="<?php the_permalink()?>" rel="bookmark" title="<?php the_title(); ?>">
                                <?php if (  get_the_post_thumbnail()  ){ the_post_thumbnail('edupress-related-posts' ); } else { ?> <img src="<?php echo get_template_directory_uri().'/images/img-not-available.jpg';?>" /><?php }?></a>
                                <h4><a href="<?php the_permalink()?>" rel="bookmark" title="<?php the_title(); ?>"><?php the_title(); ?></a></h4>
                                <p class="meta"><?php esc_html_e( 'In', 'edupress'); ?> "<?php echo get_the_category_list( esc_html__( ', ', 'edupress' ) );?>"</p>
                            </article>
                        </div>
                        <?php }?>
					...
					?>
                     ...
                     <section class="about-author clearfix">
                        ...
                        <a href="<?php echo  esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) );?>" class="btn btn-ex-small btn-orange"><?php esc_html_e('View All Post','edupress');?></a>
                        </div>
