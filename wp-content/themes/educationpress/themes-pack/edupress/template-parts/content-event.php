...
<article id="post-<?php the_ID(); ?>" class="events" >
     ...
     <!-- Image Post -->
	    <?php edupress_standard_thumbnail();?>
	...
	<header class="entry-header">
		...
		<div class="entry-meta clearfix">
			...
			<?php esc_html_e('on:','edupress'); ?> <strong><?php echo  esc_html(mysql2date( get_option( 'date_format' ), get_post_meta( $post->ID, 'EDU_PRESS_event_start_datetime', true ))) ;?></strong> <small><?php esc_html_e('.','edupress'); ?></small> 
			 <?php endif;?>
            <?php if( !empty( $event_location ) ) :?>
			<?php esc_html_e('at:','edupress'); ?> <strong><?php echo  esc_html(get_post_meta( $post->ID, 'EDU_PRESS_event_location', true ));?></strong><small><?php esc_html_e('.','edupress'); ?></small>
			<?php endif;?>
             <?php if( !empty( $event_start_datetime ) ) :?>
			<?php esc_html_e('Time:','edupress'); ?> <strong><?php echo  esc_html(mysql2date( get_option( 'time_format' ), get_post_meta( $post->ID, 'EDU_PRESS_event_start_datetime', true ))) ;?></strong></p>
            <?php endif;?>
	...
	if($edupress_options['edupress_eventdetail_countdown']){
    ...
    <?php
    	...
    	<div class="col-xs-12 col-sm-8" id="timeleft_<?php echo esc_attr($post->ID.$milliseconds);?>">
         ...
         <?php 
		  ...
		  if ( ! empty( $event_url ) ) {?>
        <a href="<?php echo esc_url($event_url);?>" class="btn btn-medium btn-normal"> <?php esc_html_e('Join Event','edupress'); ?></a>
         <?php }?>
	...
	if($expire_date < date('Y-m-d'))
	{?>
    <div class="alert alert-warning"><a aria-label="Close" data-dismiss="alert" class="close"><span aria-hidden="true">&times;</span></a> <?php esc_html_e('This event is expired.','edupress'); ?></div>
    <?php
	}?>
    ...
    <?php if($edupress_options['edupress_eventdetail_map']){?>
<?php 
