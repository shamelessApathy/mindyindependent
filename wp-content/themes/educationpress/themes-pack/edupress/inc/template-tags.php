...
if ( ! function_exists( 'edupress_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function edupress_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( ' %s', 'post date', 'edupress' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'edupress' ),
...
if ( ! function_exists( 'edupress_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function edupress_entry_footer() {
	// Hide category and tag text for pages.
	if ( 'post' === get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
		$categories_list = get_the_category_list( esc_html__( ', ', 'edupress' ) );
		if ( $categories_list && edupress_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in &quot; %1$s &quot; ', 'edupress' ) . ' <small>.</small> </span> ', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'edupress' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tags &quot; %1$s &quot;', 'edupress' ) . ' <small>.</small> </span> ', $tags_list ); // WPCS: XSS OK.
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( esc_html__( 'Leave a comment', 'edupress' ), esc_html__( '1 Comment', 'edupress' ), esc_html__( '% Comments', 'edupress' ) );
		echo ' <small>.</small> </span>';
	}

	edit_post_link( esc_html__( 'Edit', 'edupress' ), ' <span class="edit-link">', '</span>' );
}
endif;
