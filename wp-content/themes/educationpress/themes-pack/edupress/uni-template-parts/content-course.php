...
$instructors_name_with_link = edupress_university_get_instructors_name_with_link();
?>

<div class="courses-info">
  <p class="excerpt"><?php echo strip_tags( get_the_excerpt() ); ?></p>
  <p class="meta">
    <?php if($edupress_options['edupress_coursedetail_instructor']){ esc_html_e( 'Instructed by: ', 'edupress' );  echo $instructors_name_with_link; }?>
    <?php if($edupress_options['edupress_coursedetail_cat']){ 
	echo get_the_term_list( get_the_ID(), 'course-category', esc_html__( '. in: ', 'edupress'), ', ' ); } ?>
  </p>
</div>
<!--courses-info #end  --> 
...
<?php
	  if( wp_is_mobile() )
	  {
		  ?>
<div class="courses-info">
  <?php get_template_part( 'template-parts/course-co-info' ); ?>
</div>
<?php
	  }
	  ?>
