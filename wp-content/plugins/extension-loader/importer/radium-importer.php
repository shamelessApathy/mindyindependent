	...
	class Radium_Theme_Importer {
	    ...
	    public function __construct() {
			...
			if( is_admin() && $pagenow == 'themes.php' && !empty( $_GET['page'] )
			 && $_GET['page']== 'EduPress_demo_installer')
	    ...
	    public function add_admin() {
	        ...
	        add_submenu_page('themes.php', "EducationPress Import Demo Data", "Import Demo Data", 'switch_themes', 'educationpress_demo_installer', array($this, 'demo_installer'));
    	...
    	public function intro_html() {
			...
			<div style="background-color: #F5FAFD; margin:10px 0;padding: 5px 10px;color: #0C518F;border: 2px solid #CAE0F3; clear:both; width:90%; line-height:18px;">
			 ...
			 <div style="background-color: #F5FAFD; margin:10px 0;padding: 5px 10px;color: #0C518F;border: 2px solid #CAE0F3; clear:both; width:90%; line-height:18px;"><p class="tie_message_hint">Before you begin, make sure all the required plugins are activated. <br/>Courses which are imported by demo import will work with marketpress only. However, You can sell courses either by marketpress or woocommerce using EducationPress theme.</p></div>
			 ...
			 if( !empty($this->imported_demos) ) { ?>
			  	...
			  	<div style="background-color: #FAFFFB; margin:10px 0;padding: 5px 10px;color: #8AB38A;border: 2px solid #a1d3a2; clear:both; width:90%; line-height:18px;">
			  		<p><?php _e('Demo already imported', 'exten-load'); ?></p>
			  	</div><?php
			   	//return;
	    ...
	    public function demo_installer() {
			...
			if( !empty($this->imported_demos ) ) {
				...
				$button_text = __('Import Again', 'exten-load');

			} else {

				$button_text = __('Import Demo Data', 'exten-load');

			}

	        ?><div id="icon-tools" class="icon32"><br></div>
	        <h2>EducationPress Import Demo Data</h2>
	    ...
	    public function set_demo_theme_options( $file ) {
			...
			if ( file_exists( $file ) ) {
				...
				if( $this->theme_options_framework == 'exten-load') {
				...
				if ( !empty( $data ) || is_array( $data ) ) {
	      		...
	      		wp_die(
      				__( 'Theme options Import file could not be found. Please try again.', 'exten-load' ),
      				'',
	    ...
	    function process_widget_import_file( $file ) {
	    	...
	    	if ( ! file_exists( $file ) ) {
	    		wp_die(
	    			__( 'Widget Import file could not be found. Please try again.', 'exten-load' ),
	    			'',
	    			array( 'back_link' => true )
	    		);
	    	}
	    ...
	    public function import_widgets( $data ) {
	    	...
	    	foreach ( $data as $sidebar_id => $widgets ) {
	    		...
	    		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
	    			...
	    			$sidebar_message = __( 'Sidebar does not exist in theme (using Inactive)', 'exten-load' );
	    		}
	    		...
	    		foreach ( $widgets as $widget_instance_id => $widget ) {
	    			...
	    			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
	    				...
	    				$widget_message = __( 'Site does not support widget', 'exten-load' ); // explain why widget not imported
	    			}
	    			...
	    			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
	    				...
	    				foreach ( $single_widget_instances as $check_id => $check_widget ) {
	    					...
	    					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
	    						...
	    						$widget_message = __( 'Widget already exists', 'exten-load' ); // explain why widget not imported
	    			...
	    			if ( ! $fail ) {
	    				...
	    				if ( $sidebar_available ) {
	    					...
	    					$widget_message = __( 'Imported', 'exten-load' );
	    				} else {
	    					$widget_message_type = 'warning';
	    					$widget_message = __( 'Imported to Inactive', 'exten-load' );
	    				}
	    			...
	    			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : __( 'No Title', 'exten-load' ); // show "No Title" if widget instance is untitled
