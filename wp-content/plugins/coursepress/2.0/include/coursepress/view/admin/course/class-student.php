...
class CoursePress_View_Admin_Course_Student {
	...
	public static function render() {
		...
		if ( count( $list_course->items ) ) {
			...
			$content .= sprintf( '<th class="row">%s</th>', __( 'Send to', 'cp' ) );
			$content .= '<td><select name="send_to" class="widefat">';
			$send_options = array(
				'all' => __( 'All students', 'cp' ),
				'all_with_submission' => __( 'All students with submissions', 'cp' ),
			);
			$units = CoursePress_Data_Course::get_units( $course_id );

			if ( ! empty( $units ) ) {
				foreach ( $units as $unit ) {
					$send_options[ $unit->ID ] = __( sprintf( 'All students with submissions of &rarr; %s', $unit->post_title ), 'cp' );
				}
			}
