...
class CoursePress_View_Front_Facilitator {
	...
	public static function parse_request( &$wp ) {
		if ( array_key_exists( 'facilitator_username', $wp->query_vars ) ) {
			...
			$content = '';
			if ( empty( $facilitator ) ) {
				$content = __( 'The requested facilitator does not exists', 'cp' );
			}

			self::$last_facilitator = empty( $facilitator ) ? 0 : $facilitator->ID;

			$page_title = ! empty( self::$last_facilitator ) ? CoursePress_Helper_Utility::get_user_name( self::$last_facilitator, false, false ) : __( 'Facilitator not found.', 'cp' );
			$args = array(
		...
		if ( $course_invite ) {
			...
			if ( $is_verified ) {
				...
				if ( ! is_user_logged_in() ) {
					...
					$args = array(
						'show_title' => false,
						'title' => apply_filters( 'coursepress_facilitator_invitation_title', esc_html__( 'Facilitator Invitation', 'cp' ) ),
	...
	public static function modal_view() {
		...
		<script type="text/template" id="modal-view4-template" data-type="modal-step" data-modal-action="facilitator-verified">
			...
			<div class="bbm-modal__section">
				<p><?php esc_html_e( 'Congratulations. You are now a facilitator of this course. ', 'cp' ); ?></p>
			</div>
